...
SESSION_SECRET = os.getenv("ODESSA_SESSION_SECRET", "").strip()
if not SESSION_SECRET:
    raise RuntimeError("ODESSA_SESSION_SECRET environment variable must be set")
...
# ── Modo local ───────────────────────────────────────────────────────────────
# O projeto é executado localmente por padrão. Um deploy pode reativar a
# autenticação definindo ODESSA_AUTH_DISABLED=0.
AUTH_DISABLED = os.getenv("ODESSA_AUTH_DISABLED", "1").strip().lower() in {"1", "true", "yes", "on"}


def require_admin(request: Request) -> dict[str, Any]:
    """Dependência FastAPI: exige sessão admin válida (cookie ou Bearer).

    No modo local (ODESSA_AUTH_DISABLED=1) permite tudo.
    """
    if AUTH_DISABLED:
