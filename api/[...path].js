  ...
  if (path === '/workflow/draft') {
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      const hasData =
        ...
        workflow.idleVideoId;
      if (!hasData && !workflow.planningCanvas) {
        return json(res, 400, {
          ok: false,
          detail: 'Payload vazio ou invalido. Verifique o JSON enviado.',
        });
      }
      const config = loadCloudConfig() || {};
  ...
  if (path.startsWith('/obs/')) {
    ...
    if (req.method === 'POST') {
      ...
      if (actionPath === 'start-live') {
        if (!enrichedPayload.stageUrl && obsSettings.stageUrl)
          enrichedPayload.stageUrl = obsSettings.stageUrl;
        if (!enrichedPayload.startupSceneName && obsSettings.startupSceneName)
          enrichedPayload.startupSceneName = obsSettings.startupSceneName;
        if (!enrichedPayload.liveSceneName && obsSettings.liveSceneName)
          enrichedPayload.liveSceneName = obsSettings.liveSceneName;
        if (!enrichedPayload.stageSourceName && obsSettings.stageSourceName)
          enrichedPayload.stageSourceName = obsSettings.stageSourceName;
        if (!enrichedPayload.chatSourceName)
          enrichedPayload.chatSourceName = obsSettings.chatSourceName || obsSettings.ocrSourceName;
        if (!enrichedPayload.canvasWidth && obsSettings.canvasWidth)
          enrichedPayload.canvasWidth = obsSettings.canvasWidth;
        if (!enrichedPayload.canvasHeight && obsSettings.canvasHeight)
          enrichedPayload.canvasHeight = obsSettings.canvasHeight;
      }
