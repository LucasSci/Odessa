...
- Endpoint backend `/ai/respond` para gerar resposta sem expor chave Gemini no bundle.
- Endpoint backend `/ai/decide` para gerar decisao estruturada do Autopilot.
...
- Persona manual gera resposta via backend quando `GEMINI_API_KEY` esta configurada.
