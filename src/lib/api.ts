...
const rawApiBaseUrl = (
  usesSameOriginCloudApi ? defaultApiBaseUrl : env?.VITE_API_BASE_URL || defaultApiBaseUrl
).replace(/\/$/, '');
...
export function apiUrl(path: string) {
  ...
  if (
    ...
    (normalized === '/obs' || normalized.startsWith('/obs/') || normalized.startsWith('/obs?'))
  ) {
  ...
  if (
    API_V1_PREFIXES.some((prefix) => normalized === prefix || normalized.startsWith(`${prefix}/`))
  ) {
