...
export default function PersonaStudio({
  ...
  }, [activePlayer, autoPlayNext, isTransitioning, fetchNextVideoId, transitionToVideo, currentVideo]);
  ...
  useEffect(() => {
    if (personaConfig && 'triggers' in personaConfig && Array.isArray((personaConfig as any).triggers)) {
      ...
      if (triggerType === 'gift') {
        ...
        try {
          ...
          const executedVideoId =
            data.executions?.find((execution: any) => execution?.action?.videoId)?.action?.videoId ||
            data.executions?.find((execution: any) => execution?.videoState?.current_video_id)?.videoState?.current_video_id ||
      ...
      {!isCleanMode && (
        <div className="flex items-center gap-2 mb-2 border-b border-slate-800 pb-2">
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wider transition",
              activeTab === 'library' ? "bg-blue-600 text-white" : "text-slate-400 hover:bg-slate-800"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wider transition",
              activeTab === 'triggers' ? "bg-blue-600 text-white" : "text-slate-400 hover:bg-slate-800"
            )}
          >
        ...
        <div className="grid grid-cols-12 gap-6 animate-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="col-span-5 flex flex-col gap-3 h-full">
            ...
            <div className="grid grid-cols-2 gap-2 shrink-0">
              <button
                onClick={() => handleTrigger('gift', { giftName: 'Rosa', sender: 'Lucas', quantity: 1 })}
              ...
              <button
                onClick={() => handleTrigger('gift', { giftName: 'Rosa', sender: 'Ana', quantity: 5 })}
                className="flex items-center justify-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 py-2 text-[10px] font-bold text-amber-300 hover:bg-amber-500/20 transition"
              >
                🌹 Rosa x5
              </button>
              <button
                onClick={() => handleTrigger('gift', { giftName: 'Coroa', sender: 'BrunoTech', quantity: 1 })}
                className="flex items-center justify-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 py-2 text-[10px] font-bold text-amber-300 hover:bg-amber-500/20 transition"
              >
                👑 Coroa
              </button>
              <button
                onClick={() => handleTrigger('gift', { giftName: 'Diamante', sender: 'CamilaBR', quantity: 1 })}
                className="flex items-center justify-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 py-2 text-[10px] font-bold text-amber-300 hover:bg-amber-500/20 transition"
              >
                💎 Diamante
              </button>
            ...
            <div className="flex-1 min-h-[200px] flex flex-col bg-[#080a0f] border border-amber-500/15 rounded-xl overflow-hidden mt-1">
              <div className="flex items-center gap-2 bg-[#0d0f14] px-3 py-2 border-b border-amber-500/15">
                ...
                <span className="text-[10px] font-bold text-amber-300/80 uppercase tracking-widest">Pipeline de Gifts</span>
                {pipelineLogs.length > 0 && (
                  <span className="ml-auto text-[9px] text-amber-400/50">{pipelineLogs.length} eventos</span>
                )}
              </div>
                      ...
                      {log.steps.map((step, si) => (
                        <div key={si} className="flex items-center gap-1.5 font-mono">
                          <span className={cn(
                            ...
                            step.status === 'blocked' ? 'bg-amber-500' : 'bg-red-500',
                          )} />
